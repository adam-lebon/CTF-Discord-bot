...
const sanitizeTextChan = (name: string) =>
    name
        .toLowerCase()
        .replace(/@/g, "a")
        .replace(/[&<>\.\/\s'::\[\]]/g, "-")
 ...
 * @param teamName
 */
export const assignUsersToTeam = async (
    guild: Guild,
    teamName: string,
    members: GuildMember[]
) => {
    ...
    await Promise.all(
        members.map((member) => {
            ...
            logger.debug(
                `Adding role "${teamRole.name}" to user @${member.displayName}`
            );
            return member.roles.add([flagerzRole, teamRole]);
        })
    );
...
export const createChannels = async (
    ...
    const existingCategory = guild.channels.cache.find(
        (chan) =>
            chan.name === teamRole.name.toUpperCase() &&
            chan.type === "category"
    );
...
export const deleteChans = async (guild: Guild) => {
    const category = guild.channels.cache.find(
        (chan) => chan.name === "TEAM" && chan.type === "category"
    );

    if (category) {
        logger.debug("Parent channel is", category.name);
        const toDelete = guild.channels.cache
            .filter((channel) => channel.parentID === category.id)
            .array();

        logger.info(
            "The following channels will be deleted",
            toDelete.map((chan) => chan.name)
        );
